document.addEventListener('DOMContentLoaded', () => {
    ...
    animalOnlineGameBtn.addEventListener('click', () => {
        ...
        // Khởi tạo chế độ chơi trực tuyến Cờ Thú
        if (typeof window.startAnimalChessOnline === 'function') {
            window.startAnimalChessOnline();
        } else {
            // Tải script nếu chưa có
            const script = document.createElement('script');
            script.src = 'animal-chess-online.js';
            script.onload = () => {
                if (typeof window.startAnimalChessOnline === 'function') {
                    window.startAnimalChessOnline();
                } else {
                    showNotification('Không thể tải mô-đun chơi trực tuyến Cờ Thú', true);
                }
            };
            script.onerror = () => {
                showNotification('Không thể tải mô-đun chơi trực tuyến Cờ Thú', true);
